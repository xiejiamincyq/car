...
const EXPECTED_ASSETS := {
	...
	"res://assets/vehicles/player_pulse_gt.png": Vector2i(96, 112),
	"res://assets/vehicles/player_driftwing.png": Vector2i(96, 112),
	"res://assets/vehicles/player_flashpoint.png": Vector2i(96, 112),
	"res://assets/vehicles/player_comet_rs.png": Vector2i(96, 112),
	"res://assets/vehicles/player_tidebreaker.png": Vector2i(96, 112),
	"res://assets/vehicles/player_aurora_x.png": Vector2i(96, 112),
...
func _run() -> void:
	...
	assert(manifest is Dictionary and manifest.size() == 12, "The vehicle sprite manifest must cover six player and six traffic vehicle sprites")
	...
	for vehicle in VehicleCatalog.all():
		assert(main._player_texture_for_id(vehicle.id).resource_path == String(vehicle.texture_path), "Player rendering must resolve the selected %s sprite" % vehicle.id)
	main.queue_free()
	await process_frame
	quit()
