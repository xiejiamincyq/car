...
func _run() -> void:
	...
	assert(track_main.current_environment_left[0].resource_path.ends_with("storm_ridge_left.png") and track_main.current_environment_left.size() == 2, "The selected track must load both left environment variants")
	assert(track_main.current_environment_right[0].resource_path.ends_with("storm_ridge_right.png") and track_main.current_environment_right.size() == 2, "The selected track must load both right environment variants")
	track_main.free()
	quit()
