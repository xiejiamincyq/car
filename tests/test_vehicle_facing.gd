...
func _init() -> void:
	for car in Catalog.all():
		...
		assert(Profile.visual_size(car, sprite.get_size()).is_equal_approx(Vector2(92, 136)))
	...
	assert(size.is_equal_approx(Vector2(92, 136)), "Approved width preview adds 15 percent width without changing length")
	quit()
