...
func _run() -> void:
	...
	assert(easy.fuel_drain_multiplier == 0.8 and normal.fuel_drain_multiplier == 1.2 and hard.fuel_drain_multiplier == 2.0, "Fuel difficulty multipliers must match the approved values")
	for index in range(3):
		var fuel_run := RunState.new(100.0, 4.0, 30.0)
		fuel_run.configure_difficulty(DifficultyProfile.for_index(index))
		fuel_run.configure_difficulty(DifficultyProfile.for_index(index))
		fuel_run.start()
		fuel_run.tick(1.0, 400.0, 760.0, 100.0)
		var expected_loss: float = 4.0 * [0.8, 1.2, 2.0][index] * RunState.fuel_load(400.0, 760.0, 100.0)
		assert(is_equal_approx(100.0 - fuel_run.fuel, expected_loss), "Actual fuel loss must apply difficulty exactly once, even after reconfiguration")
