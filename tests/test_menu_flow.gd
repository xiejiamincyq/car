...
func _run() -> void:
	...
	var garage = main.get_node("CanvasLayer/VehicleSelectScreen")
	assert(tour_map.visible and not title_screen.visible, "Start must open the keyboard tour map")
	assert(tour_map.confirm_selection(), "The unlocked starter track must open the garage")
	assert(garage.visible and not tour_map.visible, "Track confirmation must open vehicle selection")
	assert(garage.confirm_selection(), "The unlocked starter vehicle must start the race")
	assert(main.run.phase == main.RunState.Phase.COUNTDOWN, "Vehicle confirmation must enter the safety countdown")
	assert(main.get_node("CanvasLayer/CountdownScreen").visible and not title_screen.visible, "Countdown must replace the garage")
	main._process(3.0)
