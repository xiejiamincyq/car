...
func _init() -> void:
	...
	paused_run.toggle_pause()
	paused_run.tick(3.0, 0.0, 760.0)
	paused_run.register_collision()
	assert(paused_run.collisions == 1, "An effective collision must be recorded for later driving-quality scoring")
	paused_run.fail_integrity()
	assert(paused_run.phase == RunState.Phase.GAME_OVER and paused_run.failure_reason == &"integrity", "Falling below the integrity limit must end the run with an explicit reason")
	paused_run.reset()
	assert(paused_run.phase == RunState.Phase.READY and is_zero_approx(paused_run.distance) and paused_run.score == 0 and paused_run.coins == 0 and paused_run.collisions == 0 and paused_run.failure_reason == &"" and is_equal_approx(paused_run.fuel, 100.0), "Reset must clear all per-run state")
	quit()
