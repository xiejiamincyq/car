...
func _init() -> void:
	...
	assert(is_equal_approx(VehicleVisualAnimation.traffic_lane_change_rotation(0.1, 1.0, true), deg_to_rad(15.0)), "An NPC changing right must immediately use the same full lean as the player")
	...
	assert(is_equal_approx(VehicleVisualAnimation.traffic_lane_change_rotation(1.9, 1.0, true), deg_to_rad(-15.0)), "An NPC changing left must keep its lean until it reaches the target")
